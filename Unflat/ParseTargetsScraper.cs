...
using System.Text;
using System.Data;
using static Unflat.ParseTargetsScraper;
...
internal static class ParseTargetsScraper
{
                ...
                switch (binaryStatementKind)
                {
                    ...
                    case SyntaxKind.CoalesceExpression:
                            ...
                            && namedType.ConstructedFrom.SpecialType == SpecialType.System_Nullable_T)
                        {
                            ...
                            // @Incomplete
                            if(!_sharedTypes.TypeByName.TryGetValue(typeArg.Name, out var nullableType))
                            {
                                throw new NotImplementedException();
                            }
                            ...
                            instructions.Add(new()
                            {
                                ...
                                CallingMethod = equals //callingGetter
            ...
            for(var i = 0; i < ilInstructs.Count; i++)
            {
                ...
                switch (code)
                {
                    ...
                    case IlOperation.Return:
                        il.Emit(OpCodes.Ret);
                        break;
                    case IlOperation.CallForHasValue:
                        il.Emit(OpCodes.Call, instruct.CallingMethod);
                        break;
                    case IlOperation.Call:
                        il.EmitCall(OpCodes.Call, instruct.CallingMethod, Type.EmptyTypes);
                        break;
                    case IlOperation.Callvirt:
                        il.EmitCall(OpCodes.Callvirt, instruct.CallingMethod, Type.EmptyTypes);
                        break;
    ...
    public static MatchingModel? Transform(GeneratorAttributeSyntaxContext gen, CancellationToken token)
    {
        ...
        if(gen.TargetNode is ClassDeclarationSyntax classDecl)
        {
            foreach(var node in classDecl.Members)
            {
                if(node is MethodDeclarationSyntax methodDecl)
                {
                    TraverseMethod(methodDecl, gen.SemanticModel);
                }
            }

            //var methods = classDecl.ChildNodes()
            ...
            //{
            //    var body = method.Body?.ChildNodes().ToList();
            //}
        }
